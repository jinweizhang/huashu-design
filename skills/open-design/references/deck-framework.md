...
Decks regress when each turn re-authors the scale-to-fit logic, the keyboard handler, the slide visibility toggle, the counter, and the print rules. Open Design ships a **fixed framework**: 1920×1080 canvas, scale-to-fit, prev/next + counter, capture-phase keyboard, click-anywhere focus, localStorage position restore, and a print stylesheet that emits a multi-page vertical PDF on Save-as-PDF — all baked in.
...
If you find yourself writing `<style>` rules for `.deck-shell`, `.deck-stage`, `.slide`, `.canvas`, `fit()`, `@media print`, or a keyboard handler — STOP. The framework already has them.
...
- Don't write your own `fit()` function or `transform: scale()` script. The framework already does it, and ad-hoc versions drift inside nested transform wrappers.
...
## Canonical skeleton (copy this verbatim)
